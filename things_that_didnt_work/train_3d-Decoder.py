...
from gqvr.model.vae import AutoencoderKL, Decoder_3D, Decoder3D_AutoencoderKL
...
def compute_stage3_3D_loss_streaming(zs, gts, vae, lpips_model, raft_model, loss_mode, scales, chunk_T=1):
    ...
    do_unified_rmse = ("unified"    in loss_mode)
    ...
    prev_gt  = None
    z_flattened = zs.to(torch.float32).reshape(B*T, Cz, Hz, Wz)
    decoded = vae.decode(z_flattened)
    ...
    for t0 in range(0, T, chunk_T):
        ...
        for i in range(tc):
            t = t0 + i
            dec_t = decoded[t, ...].unsqueeze(dim=0)
            gt_t  = gts[:, t, ...]   # [1,3,H,W]
            # print("dec_t.shape:", dec_t.shape, "gt_t.shape:", gt_t.shape)
            ...
            if do_flow and prev_dec is not None:
                # print("doing flow / content fixing loss")
            ...
            if do_unified_rmse and prev_dec is not None:
                # print("doing unified / frame loss")
    ...
    if do_l1:
        total_loss = total_loss + total_l1 
    if do_lpips:
        total_loss = total_loss + total_lp
    if do_unified_rmse:
        total_loss = total_loss + total_rmse
    loss_dict = {
        "flow_loss": float(total_flow.detach().item()),
        "l1_loss": float(total_l1.detach().item()),
        "perceptual": float(total_lp.detach().item()),
        "unified_rmse": float(total_rmse.detach().item())
    }
...
def main(args) -> None:
    ...
    vae = Decoder3D_AutoencoderKL(cfg.model.vae_cfg.ddconfig, cfg.model.vae_cfg.embed_dim)
        ...
        for batch in loader:
            ...
            if cfg.dataset.train.params.precomputed_latents:
                ...
                with torch.amp.autocast("cuda", dtype=torch.float16):
                    loss, loss_dict = compute_stage3_3D_loss_streaming(stable_zs, gts, vae, lpips_model, raft_model=raft_model, 
            ...
            if global_step % cfg.checkpointing_steps == 0:
                if accelerator.is_local_main_process:
                    ...
                    ckpt_path = f"{ckpt_dir}/ThreeD_Decoder_{global_step:07d}.pt"
