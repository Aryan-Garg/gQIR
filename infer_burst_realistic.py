...
def main(args) -> None:
    ...
    for ididid, batch in enumerate(loader):
        with torch.inference_mode():
            ...
            lqs = batch["lqs"].permute(0, 1, 4, 2, 3) # .to(device) # B T C H W 
            print(f"Before 3-bit summing lqs.shape: {lqs.shape}")
            three_bit_lqs = []
            for i in range(0, lqs.size(1), 7):
                three_bit_lqs.append(torch.mean(lqs[:, i:i+7, ...], dim=1, keepdim=True))
            lqs = torch.cat(three_bit_lqs, dim=1).to(device)
            print(f"After 3-bit summing lqs.shape: {lqs.shape}")

            gts = batch["gts"].permute(0, 1, 4, 2, 3) # B T C H W to match decoder output
            print(f"Before 3-bit summing gts.shape: {gts.shape}")
            three_bit_gts = []
            for i in range(0, gts.size(1), 7):
                three_bit_gts.append(torch.mean(gts[:, i:i+7, ...], dim=1, keepdim=True))
            gts = torch.cat(three_bit_gts, dim=1).to(device)
            print(f"After 3-bit summing gts.shape: {gts.shape}")
            bs = lqs.size(0)
            T = lqs.size(1)
