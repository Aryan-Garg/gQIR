...
class ZeroSRCConnector(nn.Module):
    ...
    def forward(self, x, feat, control_scale=1.0):
        x = self.norm(x)
        scale = self.scale(feat) * control_scale
        shift = self.shift(feat) * control_scale
...
class ControlledUnetModel(UNetModel):
    def __init__(self, *args, **kwargs):
        ...
        for module in self.output_blocks:
            if len(module) == 3:
                ch = module[0].out_channels
                self.zeroSRC_connector.append(ZeroSRCConnector(ch, ch))  # before upsample
            ch = module[0].out_channels
            self.zeroSRC_connector.append(ZeroSRCConnector(ch, ch))    # after skip
    ...
    def forward(self, x, timesteps=None, context=None, control=None, only_mid_control=False, **kwargs):
        ...
        with torch.no_grad():
            ...
            adapter_idx = len(self.zeroSRC_connector) - 1
            control_idx = len(control) - 1 if control else None
        ...
        for i, module in enumerate(self.output_blocks):
            skip = hs.pop()
            # h = self.project_modules[adapter_idx](control[control_idx], skip, h)
            # adapter_idx -= 1
            if len(module) == 3:
                assert isinstance(module[2], Upsample)
                # Run through res blk & transformer blk
