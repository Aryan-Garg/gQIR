...
# Adapted from: https://github.com/NVlabs/SPADE/blob/master/models/networks/normalization.py#L42
class SPADE(nn.Module):
    def __init__(self, label_nc, norm_nc): 
        # label_nc == in channels; norm_nc == out channels
      
        super().__init__()

        self.param_free_norm = normalization(norm_nc)

        # The dimension of the intermediate embedding space. Yes, hardcoded in SPADE
        nhidden = 128
...
def spade(in_chan, out_chan):
    return SPADE(label_nc=in_chan, norm_nc=out_chan)