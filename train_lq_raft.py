...
from gqvr.model.core_raft.utils.flow_viz import flow_to_image
...
def main(args) -> None:
    ...
    while global_step < max_steps:
        for batch in loader:
            ...
            # print(f"Merged lq shape: {merged_lq.shape} | min: {merged_lq.min().item():.4f}, max: {merged_lq.max().item():.4f}")
            del aligned_lqs
            torch.cuda.empty_cache()
           
            # Pass merged lq through model:
            # decoded = vae.decode(vae.encode(merged_lq).mode()) # B 3 H W; fp32 decode for best quality
            decoded = center_gt # B 3 H W; dummy for no VAE case

            with torch.amp.autocast("cuda", dtype=torch.float16):
                loss, loss_dict = compute_burst_loss(center_gt, merged_lq, lpips_model, scales=cfg.loss_scales, loss_mode="gt_perceptual")
            ...
            if global_step % cfg.log_every == 0:
                lq_center = (((lq_center.cpu() + 1) / 2)*255).squeeze(0).permute(1,2,0).detach().cpu().numpy().astype("uint8")
                Image.fromarray(lq_center).save(os.path.join( exp_dir, f"lq_center_{global_step:06d}.png"))
                lq_np = (((merged_lq + 1 )/ 2) * 255.).squeeze(0).permute(1,2,0).detach().cpu().numpy().astype("uint8")
                # print(f"Merged lq_np shape: {lq_np.shape} | min: {lq_np.min().item():.4f}, max: {lq_np.max().item():.4f}")
                Image.fromarray(lq_np).save(os.path.join( exp_dir, f"lq_merged_{global_step:06d}.png"))
                flow_vi = flow_to_image(flow_bw[0].permute(1,2,0).cpu().detach().numpy())
                # print(f"Flow viz shape: {flow_vi.shape} | min: {flow_vi.min().item():.4f}, max: {flow_vi.max().item():.4f}")
                Image.fromarray(flow_vi).save(os.path.join(exp_dir, f"flow_bw_{global_step:06d}.png"))
